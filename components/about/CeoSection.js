...
export default function CeoSection({ ceoSection }) {
  ...
  const Mansoor = ceoSection?.[1]?.subfields;
  const name = Mansoor?.[1]?.value;
  const degisnation = Mansoor?.[0]?.value;
  ...
  const tdegisnation = Tahseen?.[0]?.value;
  const trole = Tahseen?.[2]?.value;
  const tlinkedin = Tahseen?.[3]?.value;
  const timage = Tahseen?.[4]?.value;
