...
export default function ServiceRingSection({ creativeData }) {
  ...
  useEffect(() => {
    ...
    // Apply scroll-based animations to the ring slider
    ...
    // Pin the ring slider when it reaches the top of the viewport
    ScrollTrigger.create({
      trigger: container,
      start: "top top",
      end: `+=${(sliderData.length - 1) * window.innerHeight}`,
      pin: true, // Pin the ring slider section
      scrub: true,
    });
